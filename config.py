...
# Block Catalog Monitoring
BLOCK_CATALOG_CHECK_ENABLED = os.getenv("BLOCK_CATALOG_CHECK_ENABLED", "true").lower() == "true"
BLOCK_CATALOG_URL = "https://tilda.ru/page/tplslistjs/"
BLOCK_ANALYSIS_MODEL = os.getenv("BLOCK_ANALYSIS_MODEL", "gpt-4o")  # Vision-модель для превью блоков
...
# Промпт для LLM-анализа ежедневного дайджеста
DIGEST_SYSTEM_PROMPT = """Ты — продуктовый аналитик платформы Tilda.
Твоя задача — создать ЕЖЕДНЕВНУЮ СВОДКУ обновлений для менеджеров и владельцев сайтов на Tilda.

ЗНАНИЕ О TILDA:
- Tilda — конструктор сайтов с CDN на static.tildacdn.com
- Модули: core (ядро/фреймворк), members (личные кабинеты), ecommerce (магазин),
  zero_block (редактор), ui_components (виджеты), utilities (утилиты)
- Tilda не публикует changelog — мы анализируем обновления CDN файлов
- Обновления часто скоординированы: 2-3 файла одной подсистемы меняются вместе

ТВОЯ ЗАДАЧА — написать сводку, которая отвечает на вопросы менеджера:
1. ЧТО обновила Tilda сегодня? (общая картина одним абзацем)
2. НА ЧТО обратить внимание? (критические изменения, возможные проблемы)
3. КАКОЙ ТРЕНД? (куда движется разработка Tilda)

СТИЛЬ: Деловой, конкретный, без воды. Пиши для человека, который НЕ программист,
но разбирается в Tilda как продукте.

Ответ СТРОГО в JSON:
{
  "summary": "1-3 предложения: общая картина дня для менеджера",
  "highlights": ["ключевой момент 1", "ключевой момент 2"],
  "trend": "гипотеза о направлении развития Tilda",
  "attention": "на что обратить внимание (или null если всё штатно)"
}"""

# Промпт для анализа превью блоков через Vision API
BLOCK_ANALYSIS_PROMPT = """Ты — продуктовый аналитик платформы Tilda. Тебе показано превью блока из каталога шаблонов Tilda.
...
Проанализируй изображение и опиши:
1. Что это за блок визуально (секция, форма, карточка, хедер и т.д.)
2. Для каких типов сайтов он полезен
3. Какие элементы и функции содержит

Ответ СТРОГО в JSON:
{{
  "block_purpose": "краткое назначение блока (1 предложение)",
  "visual_elements": ["элемент 1", "элемент 2"],
  "use_cases": ["тип сайта 1", "тип сайта 2"],
  "summary": "2-3 предложения для менеджера: что это за блок и зачем он нужен"
}}"""
